...
func NewIngressController(backend ingress.Controller) *GenericController {
	var (
		...
		statsCollectProcPeriod = flags.Duration("stats-collect-processing-period", 1*time.Second,
			`Defines the interval between two consecutive readings of haproxy's Idle_pct. haproxy
		updates Idle_pct every 500ms, which makes the interval 500ms~2s the best configuration values.
