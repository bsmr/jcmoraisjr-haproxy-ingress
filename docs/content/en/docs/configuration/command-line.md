...
| [`--stats-collect-processing-period`](#stats)           | time                       | `1s`                    | v0.10 |
...
* `--stats-collect-processing-period`: Defines the interval between two consecutive readings of haproxy's `Idle_pct`, used to generate `haproxy_processing_seconds_total` metric. haproxy updates Idle_pct every 500ms, which makes the interval `500ms~2s` the best configuration values. The default value is `1s`. Values closer to `500ms` will produce a more accurate result. Change to 0 (zero) to disable this metric.
